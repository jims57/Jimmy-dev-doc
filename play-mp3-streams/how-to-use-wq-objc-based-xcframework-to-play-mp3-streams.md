...
### 1.4 自动完成检测（v1.2.0新特性，v1.3.0优化）

XCFramework 会自动检测播放完成，无需手动调用 `notifyDataComplete`：

- **空流检测**：自动识别0字节或12字节头部的空流信号
- **超时检测**：如果5秒内无新数据，自动标记完成（v1.3.0优化）
- **精确回调**：延迟0.5秒确保硬件缓冲区播放完成后才触发 `onPlaybackCompleted`（v1.3.0优化）
...
### 1.6 多轮播放支持（v1.3.0新特性）

支持在同一WebSocket连接上进行多轮播放：

- **自动重连**：WebSocket关闭后自动重新连接
- **状态保持**：保持连接状态，无需手动重连
- **按钮状态管理**：每次播放自动重置按钮状态
- **无缝切换**：停止后立即可以开始新的播放
