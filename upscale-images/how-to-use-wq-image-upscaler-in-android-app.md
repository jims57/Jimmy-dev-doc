# 如何在Android应用中使用wq-image-upscaler-1.5.0.aar库

*作者：Jimmy Gan*

*最后更新：2025年11月19日*

本指南将详细介绍如何从零开始在Android项目中集成和使用【沃奇】图片降噪增强AAR库（wq-image-upscaler-1.5.0.aar），该库基于AI模型提供强大的图像降噪功能。
...
```bash
# AAR库文件位置
/path/to/android-use-litert/my-info/android_aar/aar-output/wq-image-upscaler-1.5.0.aar
```
...
# 复制AAR文件
cp wq-image-upscaler-1.5.0.aar /path/to/your/android/project/app/libs/
```
...
**重要提示：** wq-image-upscaler-1.5.0.aar已经内置了所有必需的依赖，包括：
...
通过遵循本指南，您应该能够成功在Android应用中集成和使用wq-image-upscaler-1.5.0.aar库，为用户提供强大的AI图像降噪功能。
---
...
从v1.6.0版本开始，AAR库自动使用JPEG压缩保存处理结果，大幅减小文件大小（80-90%）同时保持高视觉质量。

**优化效果：**
- **文件大小减少**：从17.4 MB降至1.7 MB（90%减少）
- **视觉质量**：JPEG质量90%，人眼无法区分差异
- **存储友好**：大幅减少用户设备存储空间占用

**技术细节：**
```java
// AAR内部自动使用JPEG压缩（质量90%）
bitmap.compress(Bitmap.CompressFormat.JPEG, 90, outputStream);
```

**质量对比：**
| 场景 | PNG格式 | JPEG 90%质量 | 减少比例 |
|------|---------|--------------|----------|
| 原始图像 (2.5MB) | 17.4 MB | ~1.7 MB | 90% |
| 大尺寸图像 | 30+ MB | ~3.0-4.0 MB | 87-90% |
| 小尺寸图像 | 5-10 MB | ~0.6-1.2 MB | 88-92% |

**为什么选择JPEG 90%质量：**
- 视频防抖 视觉质量：人眼无法区分与原图的差异
- 视频防抖 文件大小：比PNG小80-90%
- 视频防抖 行业标准：专业图像应用使用85-95%质量范围
- 视频防抖 降噪应用：完美保持所有降噪和增强效果
- 视频防抖 生产就绪：适合实际部署使用

**注意事项：**
- JPEG格式不支持透明度（alpha通道）
- 如需透明背景，请使用PNG格式
- 当前降噪应用不涉及透明度，可安全使用JPEG

---

## 版本历史

### 版本 1.6.0 (2025年11月22日)

- **新增JPEG压缩优化**：自动使用JPEG格式保存（质量90%）
- **文件大小减少90%**：从17.4 MB降至1.7 MB
- **保持高视觉质量**：JPEG 90%质量人眼无法区分差异
- **存储空间友好**：大幅减少用户设备存储占用
- **生产环境就绪**：适合实际部署使用
