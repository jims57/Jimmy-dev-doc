...
从v1.1.0版本开始，XCFramework自动使用JPEG压缩保存处理结果，大幅减小文件大小（80-90%）同时保持高视觉质量。

**优化效果：**
- **文件大小减少**：从17.4 MB降至1.7 MB（90%减少）
- **视觉质量**：JPEG质量90%，人眼无法区分差异
- **存储友好**：大幅减少用户iPhone存储空间占用

**技术细节：**
```swift
// XCFramework内部自动使用JPEG压缩（质量100%）
imageData = image.jpegData(compressionQuality: 1.0)
```

**智能格式选择：**
- 默认使用JPEG格式（文件扩展名为.jpg）
- 如需PNG格式，使用.png扩展名
- 自动根据文件扩展名选择压缩格式
...
```swift
// 自动使用JPEG格式（推荐）
let outputPath = documentsPath + "/upscaled_\(timestamp).jpg"

// 如需PNG格式（文件较大）
let outputPath = documentsPath + "/upscaled_\(timestamp).png"
```
...
- **智能格式选择**：根据文件扩展名自动选择JPEG或PNG
- **存储空间友好**：大幅减少用户iPhone存储占用
- **生产环境就绪**：适合实际部署使用
